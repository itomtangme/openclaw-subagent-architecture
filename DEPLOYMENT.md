...
| Plugin | Auto-loaded from skill dir (or symlinked) | Auto-enforcement |
...
| Status dashboard | `~/.openclaw/workspace/STATUS.md` | Live org health (auto-generated by plugin) |
...
Symlink the **repo root** (not a subdirectory) into your plugins directory:

```bash
# Create the plugins directory if it doesn't exist
mkdir -p ~/.openclaw/workspace/plugins

# Symlink the repo root — must be named "architecture-enforcer" to match the manifest ID
ln -s ~/.openclaw/skills/openclaw-org \
  ~/.openclaw/workspace/plugins/architecture-enforcer
```

Register it in `openclaw.json` under `plugins.entries`, and ensure `plugins.load.paths` includes the directory where the symlink lives:
...
> **Note**: Plugin config goes inside `plugins.entries.<id>.config`, not `plugins.config.<id>`.
>
> **Important**: `plugins.load.paths` tells the OpenClaw gateway where to scan for plugin directories. Without this setting, the gateway will not discover the plugin even if the symlink and `entries` config are correct.
...
> ⚠️ **Common mistake**: Do NOT use `plugins.config` (top-level). That key is not recognized by OpenClaw. Config must be nested under `plugins.entries.<plugin-id>.config`.
...
`STATUS.md` is **auto-generated** by the plugin on every enforcement pass (gateway start, `/enforce`, or CLI `enforce-architecture`). Check `~/.openclaw/workspace/STATUS.md` — it should show:
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Plugin manifest not found | Manifest not in same dir as entrypoint | Ensure `openclaw.plugin.json` is at repo root alongside `index.ts` |
| "Unsafe plugin manifest" | Symlinked manifest file | Only the directory can be symlinked, not individual files. The manifest must be a real file. |
| Plugin ID mismatch warning | Plugin dir name ≠ manifest ID | Ensure the symlink is named `architecture-enforcer` to match the manifest `id` field |
| Plugin not discovered at all | Missing `plugins.load.paths` | Add `"plugins": { "load": { "paths": ["~/.openclaw/workspace/plugins"] } }` to `openclaw.json` |
...
6. **`plugins.config` instead of `plugins.entries.<id>.config`** — Config must be nested under entries
7. **Missing `plugins.load.paths`** — The gateway needs `plugins.load.paths` to know where to scan for plugin directories. Without it, even a correctly symlinked and configured plugin won't be discovered.
...
| Enable plugin | Symlink to plugins dir + add to `plugins.load.paths` and `plugins.entries` in `openclaw.json` |
...
*Deployment Guide v1.1 — Architecture v2.2*
